...
class DomRia(webdriver.Chrome):
    ...
    def select_city(self, city: str) -> None:
        city_element = self.find_element_by_css_selector('input[placeholder="Введіть місто"]')
        city_element.click()
    ...
    def select_state(self, state: str) -> None:
        search_field = self.find_element_by_css_selector('input#autocomplete-1')
        search_field.click()
        selected_state_element = self.find_element_by_xpath(f'//label[text()="{state}"]')
        selected_state_element.click()
        time.sleep(3)

    def select_price(self, start_price: int, end_price: int) -> None:
        price_field = self.find_element_by_css_selector('div#mainAdditionalParams_0')
        price_field.click()
        
        start_price_field = self.find_element_by_css_selector('input[placeholder="Від"]')
        start_price_field.send_keys(start_price)

        end_price_field = self.find_element_by_css_selector('input[placeholder="До"]')
